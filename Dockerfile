...
RUN wget -O /tmp/install-reviewdog.sh https://raw.githubusercontent.com/reviewdog/reviewdog/fd59714416d6d9a1c0692d872e38e7f8448df4fc/install.sh && \
  sh /tmp/install-reviewdog.sh -b /usr/local/bin/ ${REVIEWDOG_VERSION} && \
  rm /tmp/install-reviewdog.sh && \
  wget -O /tmp/install-tmpl.sh https://raw.githubusercontent.com/haya14busa/tmpl/master/install.sh && \
  sh /tmp/install-tmpl.sh -b /usr/local/bin/ ${TMPL_VERSION} && \
  rm /tmp/install-tmpl.sh && \
  wget -O /tmp/install-offset.sh https://raw.githubusercontent.com/haya14busa/offset/master/install.sh && \
  sh /tmp/install-offset.sh -b /usr/local/bin/ ${OFFSET_VERSION} && \
  rm /tmp/install-offset.sh && \
  wget -O /tmp/install-ghglob.sh https://raw.githubusercontent.com/haya14busa/ghglob/master/install.sh && \
